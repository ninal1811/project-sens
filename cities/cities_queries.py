...
def load_cache():
    ...
    Unit tests should not require a running MongoDB instance, so if the DB is
    unavailable we fall back to an empty cache.
    """
    global cache
    cache = {}
...
def create(details: dict) -> str:
    if not isinstance(details, dict):
        raise ValueError("Bad details type")

    city_val = details.get(CITY)
...
def update_city(
    city_name: str, state_code: str, country_code: str, new_data: dict
) -> bool:
